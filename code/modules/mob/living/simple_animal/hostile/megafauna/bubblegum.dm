...
/mob/living/simple_animal/hostile/megafauna/bubblegum/death()
	...
	playsound(get_turf(src),'sound/magic/enter_blood.ogg', 200, 1)
	visible_message("<span class='danger'>[src]sinks into a pool of blood, fleeing the battle. You've won, for now...  .</span>")
	ghostize()
	qdel(src)
	return
...
/mob/living/simple_animal/hostile/megafauna/bubblegum/proc/blood_spray()
	...
	spawn(0)
		...
		var/range = 10
		for(var/turf/open/in getline(src,E))
			break
			playsound('sound/effects/splat.ogg', 100, 1, -1)
