/obj/item/device/portacargo
	name = "\improper Portable Cargo Console"
	desc = "The Chief of Personnel's link to the Cargo Networks"
	icon = 'icons/obj/portacargo.dmi'
	icon_state = "portacargo"
	w_class = 1
	var/temp
	var/last_viewed_group
	var/can_order_contraband
	var/hacked
	var/reqtime
	var/shuttleId = "mining"
	var/possible_destinations = "mining_home;mining_away"

/obj/item/device/portacargo/attack_self(mob/user)
	if(!allowed(user))
	...
	dat += "<HR>Mining Shuttle Status: [M ? M.getStatusText() : "*Missing*"]<br>"
	if(M)
	...
	var/datum/browser/popup = new(user, "computer", "Portable Supply Console", 700, 470)
	popup.set_content(dat)
	popup.open()
	return

/obj/item/device/portacargo/emag_act(mob/user)
	if(!hacked)
		user << "<span class='notice'>Special supplies unlocked.</span>"
		hacked = 1

/obj/item/device/portacargo/Topic(href, href_list)
	if(..())
		return
...
/obj/item/device/portacargo/proc/post_signal(command)
