...
/obj/machinery/cryopod/robot
	...
	disallow_occupant_types = list(/mob/living/simple_animal/drone)
	applies_stasis = 0
...
// This function can not be undone; do not call this unless you are sure
// Also make sure there is a valid control computer
/obj/machinery/cryopod/proc/despawn_occupant(mob/user = usr)
...
//hardcoded af but idc
	var/datum/data/record/foundrecord = find_record("name", occupant.real_name, data_core.general)
	if(foundrecord)
		qdel(foundrecord)



	// AN ATTEMPT TO INCREASE THE JOB LIMIT ONCE THEY ARE DELETED
	/*
	var/Add = occupant.job
